...
kit=bpy.data.objects['OfficeKit'];assert kit['kitId']=='restore-retro-office' and kit['assetVersion']=='1.0.0'
...
report={'assetVersion':kit['assetVersion'],'glbSha256':hashlib.sha256(open(OUT+'/office-kit.glb','rb').read()).hexdigest(),'triangles':triangles,'renderMeshes':len(allmeshes),'embeddedImages':[{'name':im.name,'size':list(im.size)}for im in images],'models':measured,'articulation':poseChecks,'checks':['Nine independent identity-transform templates and articulated notebook cover','Finite geometry, positive transforms, complete UVs and normals','Three embedded texture atlas images, no external image request','Geometry bounds and triangle counts match editable Blender source','Locker hollow center reaches inner rear wall','Drawer hollow center reaches inner bottom panel','Notebook spine and locker hinge pose sweeps','Four actual reimported-GLB Blender renders for each model']}
